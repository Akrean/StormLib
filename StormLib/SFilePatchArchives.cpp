...
static bool ExtractPatchPrefixFromFile(TStreamProviderFactory * pFactory, const TCHAR * szHelperFile, char * szPatchPrefix, size_t nMaxChars, size_t * PtrLength)
{
	TFileStream * pStream;
	...
	pStream = FileStream_OpenFile(pFactory, szHelperFile, STREAM_FLAG_READ_ONLY | (pFactory ? BASE_PROVIDER_STREAM : BASE_PROVIDER_FILE));
	if (pStream != NULL)
	{
...
static bool FindPatchPrefix_SC2_HelperFile(TMPQArchive * haBase, TMPQArchive * haPatch)
{
	...
	if (ExtractPatchPrefixFromFile(haBase->pStream->pFactory, szHelperFile, szPatchPrefix, MAX_SC2_PATCH_PREFIX, &nLength))
		bResult = CheckAndCreatePatchPrefix(haPatch, szPatchPrefix, nLength);
	...
	DWORD dwFlags)
{
	...
	if (nError == ERROR_SUCCESS)
	{
		if (SFileOpenArchive(ha->pStream->pFactory, szPatchMpqName, 0, MPQ_OPEN_READ_ONLY | MPQ_OPEN_PATCH, &hPatchMpq))
		{
