...
//-----------------------------------------------------------------------------
// Local functions - base stream provider file support

static bool BaseStream_Create(TFileStream * pStream)
{
	if (!pStream->pFactory) {
		SetLastError(ERROR_BAD_ARGUMENTS);
		return false;
	}
	if (!pStream->pFactory->Create) {
		SetLastError(ERROR_NOT_SUPPORTED);
		return false;
	}
	if (!pStream->pFactory->Create(pStream->pFactory->pData, pStream->szFileName, (pStream->dwFlags & STREAM_FLAG_WRITE_SHARE) != 0, &pStream->Base.Stream.pProvider)) return false;
	// Reset the file size and position
	pStream->Base.Stream.FileSize = 0;
	pStream->Base.Stream.FilePos = 0;
	return true;
}

static bool BaseStream_Open(TFileStream * pStream, const TCHAR * szFileName, DWORD dwStreamFlags)
{
	if (!pStream->pFactory) {
		SetLastError(ERROR_BAD_ARGUMENTS);
		return false;
	}
	if (!pStream->pFactory->Open) {
		SetLastError(ERROR_NOT_SUPPORTED);
		return false;
	}
	if (!pStream->pFactory->Open(pStream->pFactory->pData, szFileName, (dwStreamFlags & STREAM_FLAG_READ_ONLY) != 0, (dwStreamFlags & STREAM_FLAG_WRITE_SHARE) != 0, &pStream->Base.Stream.pProvider, &pStream->Base.Stream.FileSize, &pStream->Base.Stream.FileTime)) return false;
	// Reset the file position
	pStream->Base.Stream.FilePos = 0;
	return true;
}

static bool BaseStream_Read(
	TFileStream * pStream,                  // Pointer to an open stream
	ULONGLONG * pByteOffset,                // Pointer to file byte offset. If NULL, it reads from the current position
	void * pvBuffer,                        // Pointer to data to be read
	DWORD dwBytesToRead)                    // Number of bytes to read from the file
{
	if (!pStream->Base.Stream.pProvider->Read) {
		SetLastError(ERROR_NOT_SUPPORTED);
		return false;
	}
	...
	if (!pStream->Base.Stream.pProvider->Read(pStream->Base.Stream.pProvider->pData, ByteOffset, pvBuffer, dwBytesToRead, &dwBytesRead)) return false;

	// Increment the current file position by number of bytes read
	// If the number of bytes read doesn't match to required amount, return false
	pStream->Base.Stream.FilePos = ByteOffset + dwBytesRead;
	if (dwBytesRead != dwBytesToRead)
		SetLastError(ERROR_HANDLE_EOF);
	return (dwBytesRead == dwBytesToRead);
}
...
static bool BaseStream_Write(TFileStream * pStream, ULONGLONG * pByteOffset, const void * pvBuffer, DWORD dwBytesToWrite)
{
	if (!pStream->Base.Stream.pProvider->Write) {
		SetLastError(ERROR_NOT_SUPPORTED);
		return false;
	}
	...
	if (!pStream->Base.Stream.pProvider->Write(pStream->Base.Stream.pProvider->pData, ByteOffset, pvBuffer, dwBytesToWrite, &dwBytesWritten)) return false;

	// Increment the current file position by number of bytes read
	pStream->Base.Stream.FilePos = ByteOffset + dwBytesWritten;

	// Also modify the file size, if needed
	if (pStream->Base.Stream.FilePos > pStream->Base.Stream.FileSize)
		pStream->Base.Stream.FileSize = pStream->Base.Stream.FilePos;
...
* \a NewFileSize New size of the file
*/
static bool BaseStream_Resize(TFileStream * pStream, ULONGLONG NewFileSize)
{
	if (!pStream->Base.Stream.pProvider->Resize) {
		SetLastError(ERROR_NOT_SUPPORTED);
		return false;
	}

	if (!pStream->Base.Stream.pProvider->Resize(pStream->Base.Stream.pProvider->pData, NewFileSize)) return false;
	pStream->Base.Stream.FileSize = NewFileSize;
	return true;
}

static void BaseStream_Close(TFileStream * pStream)
{
	if (pStream->Base.Stream.pProvider != NULL) {
		pStream->pFactory->Close(pStream->pFactory->pData, pStream->Base.Stream.pProvider);
	}
	pStream->Base.Stream.pProvider = NULL;
}

// Initializes base functions for the disk file
static void BaseStream_Init(TFileStream * pStream)
{
	pStream->BaseCreate = BaseStream_Create;
	pStream->BaseOpen = BaseStream_Open;
	pStream->BaseRead = BaseStream_Read;
	pStream->BaseWrite = BaseStream_Write;
	pStream->BaseResize = BaseStream_Resize;
	pStream->BaseGetSize = BaseFile_GetSize; //reuse
	pStream->BaseGetPos = BaseFile_GetPos; //reuse
	pStream->BaseClose = BaseStream_Close;
}
...
static TFileStream * AllocateFileStream(
	TStreamProviderFactory * pFactory,
	...
	DWORD dwStreamFlags)
{
	...
	if (szNextFile[0] == _T('*'))
	{
		...
		pMaster = FileStream_OpenFile(pFactory, szNextFile + 1, STREAM_FLAG_READ_ONLY);
	}
...
static TFileStream * FlatStream_Open(TStreamProviderFactory * pFactory, const TCHAR * szFileName, DWORD dwStreamFlags)
{
	TBlockStream * pStream;
	ULONGLONG ByteOffset = 0;

	// Create new empty stream
	pStream = (TBlockStream *)AllocateFileStream(pFactory, szFileName, sizeof(TBlockStream), dwStreamFlags);
	if (pStream == NULL)
		return NULL;
...
static TFileStream * PartStream_Open(TStreamProviderFactory * pFactory, const TCHAR * szFileName, DWORD dwStreamFlags)
{
	TBlockStream * pStream;
...
static TFileStream * MpqeStream_Open(TStreamProviderFactory * pFactory, const TCHAR * szFileName, DWORD dwStreamFlags)
{
	TEncryptedStream * pStream;

	// Create new empty stream
	pStream = (TEncryptedStream *)AllocateFileStream(pFactory, szFileName, sizeof(TEncryptedStream), dwStreamFlags);
	if (pStream == NULL)
		return NULL;
...
static TFileStream * Block4Stream_Open(TStreamProviderFactory * pFactory, const TCHAR * szFileName, DWORD dwStreamFlags)
{
	...
	DWORD dwStreamFlags)
{
	...
	case BASE_PROVIDER_FILE:
		if (pFactory) dwStreamFlags = (dwStreamFlags & ~BASE_PROVIDER_FILE) | BASE_PROVIDER_STREAM;
		break;
	case BASE_PROVIDER_STREAM:
		if (!pFactory) {
			SetLastError(ERROR_BAD_ARGUMENTS);
			return NULL;
		}
	...
	// We only support creation of flat, local file or straems
	if (((dwStreamFlags & (STREAM_PROVIDERS_MASK)) != (STREAM_PROVIDER_FLAT | BASE_PROVIDER_FILE)) && ((dwStreamFlags & (STREAM_PROVIDERS_MASK)) != (STREAM_PROVIDER_FLAT | BASE_PROVIDER_STREAM)))
	{
		SetLastError(ERROR_NOT_SUPPORTED);
		return NULL;
	}
	...
	pStream = AllocateFileStream(pFactory, szFileName, sizeof(TBlockStream), dwStreamFlags);
	if (pStream != NULL)
	{
	...
	DWORD dwStreamFlags)
{
	...
	switch (dwProvider & BASE_PROVIDER_MASK)
	{
	case BASE_PROVIDER_FILE:
		if (pFactory) dwProvider = (dwProvider & ~BASE_PROVIDER_FILE) | BASE_PROVIDER_STREAM;
		break;
	case BASE_PROVIDER_STREAM:
		if (!pFactory) {
			SetLastError(ERROR_BAD_ARGUMENTS);
	...
	case STREAM_PROVIDER_FLAT:
		return FlatStream_Open(pFactory, szFileName, dwStreamFlags);

	case STREAM_PROVIDER_PARTIAL:
		return PartStream_Open(pFactory, szFileName, dwStreamFlags);

	case STREAM_PROVIDER_MPQE:
		return MpqeStream_Open(pFactory, szFileName, dwStreamFlags);

	case STREAM_PROVIDER_BLOCK4:
		return Block4Stream_Open(pFactory, szFileName, dwStreamFlags);
