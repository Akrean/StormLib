...
#define BASE_PROVIDER_STREAM        0x00000003  // Base data source is a TStreamProvider
...
//-----------------------------------------------------------------------------
// Types for opening Files from arbitrary streams
typedef struct _TStreamProvider TStreamProvider;
typedef struct _TStreamProviderFactory TStreamProviderFactory;

typedef bool(WINAPI * STREAM_PROVIDER_FACTORY_CREATE)(void * pData, const TCHAR * szFileName, bool ShareWrite, TStreamProvider ** ppProvider);
typedef bool(WINAPI * STREAM_PROVIDER_FACTORY_OPEN)(void * pData, const TCHAR * szFileName, bool ReadOnly, bool ShareWrite, TStreamProvider ** ppProvider, ULONGLONG* pFileSize, ULONGLONG* pFileTime);
typedef bool(WINAPI * STREAM_PROVIDER_FACTORY_CLOSE)(void * pData, TStreamProvider * pProvider);
typedef bool(WINAPI * STREAM_PROVIDER_READ)(void * pData, ULONGLONG ByteOffset, void * pvBuffer, DWORD dwBytesToRead, DWORD* pBytesRead);
typedef bool(WINAPI * STREAM_PROVIDER_WRITE)(void * pData, ULONGLONG ByteOffset, const void * pvBuffer, DWORD dwBytesToWrite, DWORD* pBytesWritten);
typedef bool(WINAPI * STREAM_PROVIDER_RESIZE)(void * pData, ULONGLONG NewSize);

typedef struct _TStreamProviderFactory
{
	void * pData;
	STREAM_PROVIDER_FACTORY_CREATE Create;
	STREAM_PROVIDER_FACTORY_OPEN Open;
	STREAM_PROVIDER_FACTORY_CLOSE Close;
} TStreamProviderFactory;

typedef struct _TStreamProvider
{
	void * pData;
	STREAM_PROVIDER_READ Read;
	STREAM_PROVIDER_WRITE Write;
	STREAM_PROVIDER_RESIZE Resize;
} TStreamProvider;

// UNICODE versions of the file access functions
TFileStream * FileStream_CreateFile(TStreamProviderFactory * pFactory, const TCHAR * szFileName, DWORD dwStreamFlags);
TFileStream * FileStream_OpenFile(TStreamProviderFactory * pFactory, const TCHAR * szFileName, DWORD dwStreamFlags);
...
bool   WINAPI SFileOpenArchive(TStreamProviderFactory* pFactory, const TCHAR * szMpqName, DWORD dwPriority, DWORD dwFlags, HANDLE * phMpq);
