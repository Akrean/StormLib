...
static bool OpenLocalFile(TStreamProviderFactory * pFactory, const char * szFileName, HANDLE * PtrFile)
{
    TFileStream * pStream;
    TMPQFile * hf = NULL;
    ...
    pStream = FileStream_OpenFile(pFactory, szFileNameT, STREAM_FLAG_READ_ONLY | (pFactory ? BASE_PROVIDER_STREAM : BASE_PROVIDER_FILE));
    if(pStream != NULL)
    {
    ...
    if(nError == ERROR_SUCCESS)
    {
        switch(dwSearchScope)
        {
            ...
            case SFILE_OPEN_LOCAL_FILE:
                ...
                // Open a local file
                return OpenLocalFile(ha ? ha->pStream->pFactory : NULL, szFileName, PtrFile); 
